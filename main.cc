...
public:
    void operator()(KVMeta& req_meta,
        ...
        cout<<"WorkerID: "<<req_meta.sender<<"   Push: "<<req_meta.push<<"   timestamp: "<<req_meta.timestamp<<": "<<num<<endl;
        if (req_meta.push){
...
void Gradient(Worker<float>& worker, vector<vector<float>>& x,
    vector<int>& y,vector<float>& w, int round=150){
...
void StartWorker(){
    ...
    vector<int> y;
    LoadData(x,y,"../data/data/split/train_x_2","../data/data/split/train_y_2");
    long t_s,t_e;
    int size=x[0].size();
    vector<float> w(size);
    t_s = clock();
    Gradient(worker,x,y,w,15);
    t_e = clock();
    float precision=Predict(x,y,w);
    cout<<"precision: "<<precision<<endl;
    cout<<"time: "<<(t_e - t_s)/100000<<endl;
